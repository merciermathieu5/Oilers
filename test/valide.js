...
(async () => {
  ...
  const drai = S.SECOURS_ROSTER.find(j => j.nom === 'Leon Draisaitl');
  egal(drai.salaire, 10500000, 'Salaire Draisaitl');
  egal(drai.ct, 1, 'Contrat Draisaitl 1 an');
  egal(S.SECOURS_Y21.length, 20, '20 patineurs dans la référence Y21 (XtraStats, validés par ratios)');
  const lundY21 = S.SECOURS_Y21.find(x => x.nom === 'Isac Lundestrom');
  egal(lundY21.pts, 95, 'Lundestrom 95 points en Y21');
  const draiY21 = S.SECOURS_Y21.find(x => x.nom === 'Leon Draisaitl');
  egal(draiY21.pts, 88, 'Draisaitl 88 points en Y21');
  egal(draiY21.hits, 142, 'Draisaitl 142 MEÉ en Y21');
  const akY21 = S.SECOURS_Y21.find(x => x.nom === 'Nicolas Aube-Kubel');
  egal(akY21.gp, 1, 'Aube-Kubel : un seul match en Y21 (transcrit tel quel)');
  ...
  egal(profilDe('Leon Draisaitl').profil, 'Elite', 'Draisaitl → Elite');
  tableauEgal(profilDe('Leon Draisaitl').stats, ['shotpct','gwg','ppg','pts','pmrang'],
    'Stats évaluées Elite = tableau 20 (PCTG, GWG, PP, P, +/-)');
  egal(profilDe('John Tavares').profil, 'Playmaker', 'Tavares → Playmaker');
  egal(profilDe('Ross Colton').profil, 'Power Forward', 'Colton → Power Forward');
  egal(profilDe('Anthony Cirelli').profil, 'Sniper', 'Cirelli → Sniper');
  egal(profilDe('Dakota Joshua').profil, 'Grinder', 'Joshua → Grinder');
  tableauEgal(profilDe('Dakota Joshua').stats, ['hits20'], 'Stats Grinder = MEÉ/20');
  egal(profilDe('Teddy Blueger').profil, 'Defensive', 'Blueger → attaquant défensif');
  egal(profilDe('Isac Lundestrom').profil, 'Two-Way Forward', 'Lundestrom → Two-Way Forward');
  tableauEgal(profilDe('Isac Lundestrom').stats, ['pts','pmrang'], 'Stats Two-Way = P, +/-');
  egal(profilDe('Anton Lundell').profil, 'Prospect Elite', 'Lundell (24 ans) → Prospect Elite');
  egal(profilDe('Anton Lundell').mat, 'ELITE', 'Prospect Elite → même matrice ELITE');
  egal(profilDe('Ozzy Wiesblatt').profil, 'Prospect Power Forward', 'Wiesblatt (23 ans) → Prospect Power Forward');
  egal(profilDe('Juuso Valimaki').profil, 'DEliteQB', 'Valimaki → DEliteQB');
  egal(profilDe('Jakob Chychrun').profil, 'DEliteShutdown', 'Chychrun → DEliteShutdown');
  egal(profilDe('Chris Tanev').profil, 'DDefensive', 'Tanev → DDefensive');
  egal(profilDe('Taige Harding').profil, 'Prospect DEliteShutdown', 'Harding (23 ans) → Prospect DEliteShutdown');
  egal(profilDe('Frederik Andersen').profil, 'Starter Goalie', 'Andersen (OV 84) → Starter Goalie');
  tableauEgal(profilDe('Frederik Andersen').stats, ['hs','svpct','qggp','ming'], 'Stats Starter = HS, SV%, QG/GP, MIN');
  egal(profilDe('Vitek Vanecek').profil, 'Backup Goalie', 'Vanecek (OV 78) → Backup Goalie');
  tableauEgal(profilDe('Vitek Vanecek').stats, ['mp','qggp','psv'], 'Stats Backup = MP, QG/GP, Psv');
  egal(profilDe('Pheonix Copley').profil, 'Backup Goalie', 'Copley (OV 78) → Backup Goalie');
  ...
  const theo = S.SECOURS_ROSTER.find(j => j.nom === 'Shea Theodore');
  const four = S.fourchetteRecote(theo, 10); // 30 ans → ModA-5 → base 65
  ...
  const jk = joueurDe('Leon Draisaitl');
  jk._profil = S.determinerProfil(jk);
  let s = S.seuilsPour(jk, 'pts', {}, {});
  tableauEgal(s.seuils, [117,104,90,77,45,-1], 'Seuils Y17 par défaut (OV 86)');
  egal(s.source, 'Y17', 'Source = Y17');
  const perso = {ELITE: {pts: {86: [120,110,100,90,50,-1]}}};
  ...
  egal(s.seuils[2], 90, 'Les autres degrés restent ceux de la matrice');
  ...
  const fixtureXtra = [
    ...
    '    [Leon Draisaitl](https://x/#Leon Draisaitl) EDMONTON        C   82  30  45  75 200  30 1700 100 2.44',
    '    [Alex Tuch](https://x/#Alex Tuch) ANAHEIM         RW  77  27  32  59 183  16 1503 243 2.38',
    '    [* Backup_RW](https://x/#Backup_RW) EDMONTON        LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'EDMONTON');
  egal(x.length, 2, 'Deux patineurs EDMONTON extraits (Anaheim exclu)');
  ...
  ok(doc.querySelector('#ficheEquipe').textContent.includes('EDMONTON'), 'Fiche d\'équipe affichée');
  ...
  egal(masse, 83690050, 'Masse salariale des 23 pros = 83 690 050 $');
  ...
  console.log('— Divers');
  egal(S.matchsEquipe(), 19, 'Fiche EDMONTON 11-8-0 → 19 matchs d\'équipe (séries Y21)');
  ...
  if (ETAT){
    ...
    ETAT.xtra = [{nom:'Leon Draisaitl', gp:9, goals:4, assists:6, pts:10, shots:25, pim:2, mp:180, hits:8}];
    const jDrai = S.SECOURS_ROSTER.find(x2=>x2.nom==='Leon Draisaitl');
    const prodRepli = S.productionDe(jDrai);
  ...
  const prodY21 = S.productionDe(S.SECOURS_ROSTER.find(x2=>x2.nom==='Isac Lundestrom'));
  egal(prodY21?._modeY21, true, 'Production servie par les données Y21 intégrées');
  egal(prodY21?.pts, 95, 'Points Y21 de Lundestrom');
  const carteLund = [...doc.querySelectorAll('#progGrille .joueur-carte')]
    .find(c=>c.querySelector('.jc-nom').textContent==='Isac Lundestrom');
  ok(!!carteLund, 'Carte de Lundestrom rendue en mode Y21');
  ok(carteLund.textContent.includes('Mode vérification Y21'), 'Note du mode Y21 sur la carte');
  const jLd = S.ETAT.roster.find(x2=>x2.nom==='Isac Lundestrom');
  const evPtsL = S.evaluerStat('pts', S.valeurStat(jLd, prodY21, 'pts', {}), S.seuilsPour(jLd, 'pts', {}, {}).seuils);
  egal(evPtsL.statut, 'memorable', 'Lundestrom Mémorable en points (95 > 68, Two-Way OV 82)');
  const jDr = S.ETAT.roster.find(x2=>x2.nom==='Leon Draisaitl');
  const evPtsD = S.evaluerStat('pts', S.valeurStat(jDr, S.productionDe(jDr), 'pts', {}), S.seuilsPour(jDr, 'pts', {}, {}).seuils);
  egal(evPtsD.statut, 'correcte', 'Draisaitl Correcte en points (77 < 88 ≤ 90, Elite OV 86)');
  ok(!carteLund.textContent.includes('proj.'), 'Aucune projection affichée (saison complète)');
  btnY21.click();
